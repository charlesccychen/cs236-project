import argparse
import os
from data import CIFAR10Dataset, Imagenet32Dataset, CaptionedImageDataset
...
from utils.utils import sample_image, sample_image2, load_model
...
parser.add_argument("--output_dir", type=str, default="outputs/samples", help="directory to store the sampled outputs")
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i, (imgs, labels, captions) in enumerate(train_loader):
            ...
            if (batches_done + 1) % opt.sample_interval == 0:
                ...
                model = model.eval()
                sample_image2(model, embedder, opt.output_dir, n_row=2, n_col=6,
...
class MyImageNetLabelsDataset(CaptionedImageDataset):
    def __init__(self, cls=None):
        self.labelIds = ['n2958343','n2880546','n2031934','n2108254','n4202417','n3128519','n6874019','n3323703','n2101108','n2121808','n3484931','n7882497','n3419014','n2898711','n1823013','n3001627','n2087551','n4531098','n4078574','n1916925','n2395003','n2206270','n1802721']
        self.labelNames = ['car','bowed stringed instrument','snipe','mastiff','shop','cream','light','fastener','spaniel','domestic cat','hand glass','concoction','garment','bridge','cuckoo','chair','hound','vessel','reservoir','stony coral','swine','hymenopterous insect','phasianid']
        # self.labelIds = ['n2121808'] * 32
        if cls:
            self.labelNames = ['n2958343'] * 32
            self.labelNames = [cls] * 32
...
if __name__ == "__main__":
    ...
    #     shuffle=True,
    # )
    val_dataset = MyImageNetLabelsDataset(cls=opt.label)
    ...
    # print('train_dataset.image_shape', train_dataset.image_shape)
    train_dataset_image_shape = (3, 32, 32)
    generative_model = ConditionalPixelCNNpp(embd_size=encoder.embed_size, img_shape=train_dataset_image_shape,
                                             nr_resnet=opt.n_resnet, nr_filters=opt.n_filters,
                                             nr_logistic_mix=3 if train_dataset_image_shape[0] == 1 else 10)
