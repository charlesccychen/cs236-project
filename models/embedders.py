...
class GloveEncoder(Embedder):
    def __init__(self):
        ...
        self.embeddings = {}
        files = ['datasets/ImageNet32/map_clsloc_synthesized_glove.json']
        for f in files:
            data = json.load(open(f, 'r'))
            for label, floats in data.items():
                self.embeddings[label] = np.array(floats, dtype=np.float64)
    ...
    def forward(self, class_labels, captions):
        ...
        for caption in captions:
            ...
            assert label in self.embeddings, label
            batch.append(self.embeddings[label])
        embeddings = np.array(batch)
        result = torch.from_numpy(embeddings).float().to(class_labels.device)
        return result
